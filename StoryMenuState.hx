...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var storything:FlxSprite = new FlxSprite().loadGraphic(Paths.image("menu/storymenuthing"));
        storything.frames = Paths.getSparrowAtlas('menu/storymenuthing');
        storything.scrollFactor.set(0, 0);
		storything.scale.x = 1.01;
		storything.scale.y = 1.01;
        storything.animation.addByPrefix('storymenu move', "storymenu move", 10);
        storything.animation.play("storymenu move");
		storything.updateHitbox();
		storything.screenCenter(X);
		storything.antialiasing = FlxG.save.data.highquality;
		add(storything);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			if(!isLocked || !weekFile.hiddenUntilUnlocked)
			{
				...
				var weekThing:MenuItem = new MenuItem(0, bgSprite.y + 596, WeekData.weeksList[i]);
				//weekThing.y += ((weekThing.height + 20) * 10);
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.71, bgSprite.y + 185).loadGraphic(Paths.image('Menu_Tracks'));
		tracksSprite.antialiasing = ClientPrefs.globalAntialiasing;
		add(tracksSprite);

		txtTracklist = new FlxText(FlxG.width + 1000, tracksSprite.y + 60, 0, "", 32);
		txtTracklist.x = 1000;
...
if (!movedBack && !selectedWeek)
	...
	function updateText()
	{
		...
		txtTracklist.x += FlxG.width * 0.28;
